        ...
        if (settings->nmea_l)
        {
          ...
          if (distance < ALARM_ZONE_NONE)
          {
            ...
            snprintf_P(NMEABuffer, sizeof(NMEABuffer),
                       ...
                       ltrim(str_climb_rate), Container[i].aircraft_type PFLAA_EXT1_ARGS);
  ...
  if (settings->nmea_l)
  {
    ...
    if (total_objects > 0)
    {
      ...
      snprintf_P(NMEABuffer, sizeof(NMEABuffer),
                 ...
                 ALARM_TYPE_AIRCRAFT, HP_alt_diff, (int)HP_distance, HP_addr PFLAU_EXT1_ARGS);
    }
    else
    {
      snprintf_P(NMEABuffer, sizeof(NMEABuffer),
                 PSTR("$PFLAU,0,%d,%d,%d,%d,,0,,," PFLAU_EXT1_FMT "*"),
                 has_Fix && (settings->txpower != RF_TX_POWER_OFF) ? TX_STATUS_ON : TX_STATUS_OFF,
                 has_Fix ? GNSS_STATUS_3D_MOVING : GNSS_STATUS_NONE,
                 power_status, HP_alarm_level PFLAU_EXT1_ARGS);
    }
...
void NMEA_Position()
{
  ...
  if (settings->nmea_g)
  {
    ...
    size_t gen_sz = nmeaSentenceFromInfo(&nmealib_buf, &info, (NmeaSentence)(NMEALIB_SENTENCE_GPGGA | NMEALIB_SENTENCE_GPGSA | NMEALIB_SENTENCE_GPRMC));
...
void NMEA_GGA()
{
  ...
  size_t gen_sz = nmeaSentenceFromInfo(&nmealib_buf, &info, (NmeaSentence)NMEALIB_SENTENCE_GPGGA);
