...
#if defined(RASPBERRY_PI) || defined(ARDUINO_ARCH_NRF52) || defined(ESP32) || defined(ARDUINO_ARCH_RP2040)
...
void exportUISettings(JsonObject &root)
{
  switch (ui_settings.units)
  {
  case UNITS_METRIC:
    root["units"] = "METRIC";
    break;
  ...
  case PROTOCOL_GDL90:
    root["data"]="GDL90";
    break;
  case PROTOCOL_MAVLINK_1:
    root["data"]="MAV1";
    break;
  case PROTOCOL_MAVLINK_2:
    root["data"]="MAV2";
    break;
  case PROTOCOL_D1090:
    root["data"]="D1090";
  ...
  switch (ui_settings.orientation)
  {
  case DIRECTION_TRACK_UP:
    root["orientation"] = "TRACK";
    break;
  ...
  switch (ui_settings.filter)
  {
  case TRAFFIC_FILTER_OFF:
    root["filter"] = "OFF";
    break;
  case TRAFFIC_FILTER_500M:
    root["filter"] = "500M";
    break;
  case TRAFFIC_FILTER_1500M:
    root["filter"] = "1500M";
    break;
  }

  root["team"] = ui_settings.team;
}
void parseUISettings(JsonObject &root)
{
  if (root.containsKey("units"))
  {
    const char *units_s = root["units"];
  ...
  if (root.containsKey("zoom"))
  {
    const char *zoom_s = root["zoom"];
    ...
    else if (!strcmp(zoom_s, "VHIGH"))
    {
      ui_settings.zoom = ZOOM_VHIGH;
    }
  }
#if 0
  JsonVariant protocol = root["data"];
  if (root.containsKey("data"))
    const char * protocol_s = root["data"];
  ...
  if (root.containsKey("rotate"))
  {
    const char *rotate_s = root["rotate"];
  ...
  {
    const char *orientation_s = root["orientation"];
  ...
  if (root.containsKey("vmode"))
  {
    const char *vmode_s = root["vmode"];
    if (!strcmp(vmode_s, "MAIN"))
    {
      ui_settings.vmode = VIEW_MODE_MAIN;
    }
    else if (!strcmp(vmode_s, "STATUS"))
    {
  ...
  if (root.containsKey("aghost"))
  {
    const char *aghost_s = root["aghost"];
  ...
  {
    const char *filter_s = root["filter"];
  ...
  ui_settings.team = root["team"];
}
#endif /* RASPBERRY_PI || ARDUINO_ARCH_NRF52 */

#if defined(RASPBERRY_PI) || defined(ARDUINO_ARCH_NRF52) || defined(ARDUINO_ARCH_RP2040) || defined(ESP32)
void exportSettings(JsonObject &root)
{
  /*switch (eeprom_block.field.settings.mode)
  {
  ...
  /*switch (eeprom_block.field.settings.rf_protocol)
  {
  ...
  switch (eeprom_block.field.settings.band)
  {
  case RF_BAND_AUTO:
    root["band"] = "AUTO";
    break;
  ...
  switch (eeprom_block.field.settings.aircraft_type)
  {
  case AIRCRAFT_TYPE_GLIDER:
    root["aircraft_type"] = "GLIDER";
    break;
  case AIRCRAFT_TYPE_TOWPLANE:
    root["aircraft_type"] = "TOWPLANE";
    break;
  case AIRCRAFT_TYPE_POWERED:
    root["aircraft_type"] = "POWERED";
    break;
  case AIRCRAFT_TYPE_HELICOPTER:
    root["aircraft_type"] = "HELICOPTER";
    break;
  case AIRCRAFT_TYPE_UAV:
    root["aircraft_type"] = "UAV";
    break;
  case AIRCRAFT_TYPE_HANGGLIDER:
    root["aircraft_type"] = "HANGGLIDER";
    break;
  case AIRCRAFT_TYPE_PARAGLIDER:
    root["aircraft_type"] = "PARAGLIDER";
    break;
  case AIRCRAFT_TYPE_BALLOON:
    root["aircraft_type"] = "BALLOON";
    break;
  case AIRCRAFT_TYPE_STATIC:
    root["aircraft_type"] = "STATIC";
    break;
  case AIRCRAFT_TYPE_UNKNOWN:
    root["aircraft_type"] = "UNKNOWN";
    break;
  }
  switch (eeprom_block.field.settings.alarm)
  {
  ...
  switch (eeprom_block.field.settings.txpower)
  {
  ...
  switch (eeprom_block.field.settings.volume)
  {
  ...
  switch (eeprom_block.field.settings.pointer)
  {
  case DIRECTION_TRACK_UP:
    root["pointer"] = "TRACK";
    break;
  ...
  JsonObject &nmea = root.createNestedObject("nmea");
  nmea["gnss"] = eeprom_block.field.settings.nmea_g != 0;
  nmea["private"] = eeprom_block.field.settings.nmea_p != 0;
  nmea["legacy"] = eeprom_block.field.settings.nmea_l != 0;
  nmea["sensors"] = eeprom_block.field.settings.nmea_s != 0;

  switch (eeprom_block.field.settings.nmea_out)
  {
  case NMEA_OFF:
    nmea["output"] = "OFF";
    break;
  ...
  switch (eeprom_block.field.settings.gdl90)
  {
  case GDL90_OFF:
    root["gdl90"] = "OFF";
    break;
  ...
  switch (eeprom_block.field.settings.d1090)
  {
  case D1090_OFF:
    root["d1090"] = "OFF";
    break;
  ...
  switch (eeprom_block.field.settings.json)
  {
  case JSON_OFF:
    root["json"] = "OFF";
    break;
  ...
  root["stealth"] = eeprom_block.field.settings.stealth != 0;
  root["no_track"] = eeprom_block.field.settings.no_track != 0;

  root["fcor"] = eeprom_block.field.settings.freq_corr;

#if defined(USE_OGN_ENCRYPTION)
  {
    char buffer[40];
    sprintf(buffer, "%08x%08x%08x%08x", eeprom_block.field.settings.igc_key[0], eeprom_block.field.settings.igc_key[1], eeprom_block.field.settings.igc_key[2], eeprom_block.field.settings.igc_key[3]);
    root["igc_key"] = buffer;
  }
#endif
  {
    char buffer[40];
    sprintf(buffer, "%06x", eeprom_block.field.settings.device_addr & 0x00FFFFFF);
    root["device_addr"] = buffer;
    sprintf(buffer, "%06x", SoC->getChipId() & 0x00FFFFFF);
    root["hwdevice_addr"] = buffer;
  }

#ifndef EXCLUDE_WIFI
  root["wifiap"] = eeprom_block.field.settings.wifiap != 0;
  root["wifista"] = eeprom_block.field.settings.wifista != 0;
  root["ssid"] = eeprom_block.field.settings.ssid;
  root["psk"] = eeprom_block.field.settings.psk;
  root["relay_ogn"] = eeprom_block.field.settings.relay_ogn != 0;
#endif
  root["name"] = eeprom_block.field.settings.name;
  root["latitude"] = eeprom_block.field.settings.latitude;
  root["longitude"] = eeprom_block.field.settings.longitude;
  root["reportme"] = eeprom_block.field.settings.reportme;
  root["relay_rf"] = eeprom_block.field.settings.relay_rf;
}
void parseSettings(JsonObject &root)
{
  /*if (root.containsKey("mode"))
  {
    const char *mode_s = root["mode"];
  ...
  {
    const char *protocol_s = root["protocol"];
  ...
  if (root.containsKey("aircraft_type"))
  {
    const char *aircraft_type_s = root["aircraft_type"];
    ...
    else if (!strcmp(aircraft_type_s, "UNKNOWN"))
    {
      eeprom_block.field.settings.aircraft_type = AIRCRAFT_TYPE_UNKNOWN;
    }
  }
  if (root.containsKey("alarm"))
  {
    const char *alarm_s = root["alarm"];
  ...
  {
    const char *txpower_s = root["txpower"];
  ...
  {
    const char *volume_s = root["volume"];
  ...
  if (root.containsKey("pointer"))
  {
    const char *pointer_s = root["pointer"];
  ...
  }
  {
    ...
    if (nmea_g.success())
      eeprom_block.field.settings.nmea_g = nmea_g;
  }
  {
    JsonVariant nmea_p = root["nmea"]["private"];
    if (nmea_p.success())
      eeprom_block.field.settings.nmea_p = nmea_p;
  }
  {
    JsonVariant nmea_l = root["nmea"]["legacy"];
    if (nmea_l.success())
      eeprom_block.field.settings.nmea_l = nmea_l;
  }
  {
    JsonVariant nmea_s = root["nmea"]["sensors"];
    if (nmea_s.success())
      eeprom_block.field.settings.nmea_s = nmea_s;
  }
  {
    JsonVariant nmea_out = root["nmea"]["output"];
    if (nmea_out.success())
    {
      const char *nmea_out_s = nmea_out;
  ...
  if (root.containsKey("gdl90"))
  {
    const char *gdl90_s = root["gdl90"];
  ...
  if (root.containsKey("d1090"))
  {
    const char *d1090_s = root["d1090"];
  ...
  if (root.containsKey("stealth"))
    eeprom_block.field.settings.stealth = root["stealth"];
  if (root.containsKey("no_track"))
    eeprom_block.field.settings.no_track = root["no_track"];
  if (root.containsKey("fcor"))
  {
    int fc = root["fcor"];
    if (fc > 30)
    {
...
#if defined(USE_OGN_ENCRYPTION)
  if (root.containsKey("igc_key"))
  {
    char buf[32 + 1];
    strncpy(buf, root["igc_key"], sizeof(buf));
...
#endif
  if (root.containsKey("device_addr") && root.containsKey("hwdevice_addr"))
  {
    const char *hwdevice_addr_s = root["device_addr"];
    uint32_t hwdevice_addr = strtoul(hwdevice_addr_s, NULL, 16);
    if (hwdevice_addr == (SoC->getChipId() & 0x00FFFFFF))
    {
      const char *device_addr_s = root["hwdevice_addr"];
      eeprom_block.field.settings.device_addr = strtoul(device_addr_s, NULL, 16);
    }
  }
#ifndef EXCLUDE_WIFI

  if (root.containsKey("wifiap"))
    eeprom_block.field.settings.wifiap = root["wifiap"];
  if (root.containsKey("wifista"))
    eeprom_block.field.settings.wifista = root["wifista"];
  if (root.containsKey("ssid") && root.containsKey("psk"))
  {
    strncpy(eeprom_block.field.settings.ssid, root["ssid"], sizeof(eeprom_block.field.settings.ssid));
    strncpy(eeprom_block.field.settings.psk, root["psk"], sizeof(eeprom_block.field.settings.psk));
  }
  if (root.containsKey("relay_ogn"))
    eeprom_block.field.settings.relay_ogn = root["relay_ogn"];
#endif
  if (root.containsKey("name"))
    strncpy(eeprom_block.field.settings.name, root["name"], sizeof(eeprom_block.field.settings.name));
  if (root.containsKey("latitude") && root.containsKey("longitude"))
  {
    eeprom_block.field.settings.latitude = root["latitude"];
    eeprom_block.field.settings.longitude = root["longitude"];
  }
  if (root.containsKey("reportme"))
    eeprom_block.field.settings.reportme = root["reportme"];
  if (root.containsKey("relay_rf"))
    eeprom_block.field.settings.relay_rf = root["relay_rf"];
}
