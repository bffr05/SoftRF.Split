...
const rf_proto_desc_t legacy_proto_desc = {
  ...
  .payload_size    = RELAY_MAXPACKET_SIZE, //LEGACY_PAYLOAD_SIZE,
  .payload_offset  = 0,
...
bool legacy_decode(void *legacy_pkt, uint32_t timestamp,ufo_t *this_aircraft, ufo_t *fop) {

  legacy_packet_t *pkt = (legacy_packet_t *) legacy_pkt;
  //uint32_t timestamp=this_aircraft->timestamp;
#ifdef FULLUFOST
  memcpy(fop->raw, legacy_pkt, sizeof(legacy_packet_t));
#endif
  ...
  if (pkt_parity % 2) {
      Serial.printf("legacy_decode ADDR=%06X Bad Parity\n",pkt->addr);
  ...
  fop->distance = gnss.distanceBetween( ThisAircraft.latitude,ThisAircraft.longitude,fop->latitude,fop->longitude);
  //Serial.printf("distanceBetween( %f %f %f %f ) = %f\n",ThisAircraft.latitude,ThisAircraft.longitude,fop->latitude,fop->longitude,fop->distance);
  fop->bearing  = gnss.courseTo( ThisAircraft.latitude,ThisAircraft.longitude,fop->latitude,fop->longitude);
  if (fop->distance>MAXDISTANCE)
    return false;
  if (!UFO_check(fop))
    return false;
  return true;
