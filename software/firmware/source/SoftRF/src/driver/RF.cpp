...
#include <fec.h>
#include "../protocol/radio/RELAY.h"
...
//BENJAMIN
byte RxBuffer[RELAY_MAXPACKET_SIZE] __attribute__((aligned(sizeof(uint32_t))));

unsigned long TxTimeMarker = 0;
byte TxBuffer[RELAY_MAXPACKET_SIZE] __attribute__((aligned(sizeof(uint32_t))));

uint32_t tx_packets_counter = 0;
uint32_t rx_packets_counter = 0;
uint32_t rx_packet_lasttime = 0;
...
bool   (*protocol_decode)(void *, uint32_t,ufo_t *, ufo_t *);
...
#if FALSE //defined(USE_OGN_RF_DRIVER)
    ...
    if (!wait || millis() > TxTimeMarker) {
      ...
      if (memcmp(TxBuffer, RxBuffer, RF_tx_size) != 0) {
        ...
        if (settings->nmea_p) {
          ...
          StdOut.print(F(","));
          StdOut.println(Bin2Hex((byte *) &TxBuffer[0],RF_tx_size));
        }
        tx_packets_counter++;
...
uint8_t RF_Payload_Size(uint8_t protocol)
{
  switch (protocol)
  {
    case RF_PROTOCOL_LEGACY:    return LEGACY_PAYLOAD_SIZE;
    //case RF_PROTOCOL_OGNTP:     return OGNTP_PAYLOAD_SIZE;
    //case RF_PROTOCOL_P3I:       return P3I_PAYLOAD_SIZE;
    //case RF_PROTOCOL_FANET:     return FANET_PAYLOAD_SIZE;
    //case RF_PROTOCOL_ADSB_UAT:  return UAT978_PAYLOAD_SIZE;
    //case RF_PROTOCOL_SOFTRFRELAY:  return RELAY_MAXPACKET_SIZE;
    default:                    return 0;
  }
}
...
static bool sx12xx_receive()
{
  ...
  //Serial.printf("sx12xx_receive sx12xx_receive_active=%u power_save=%u\n",sx12xx_receive_active,settings->power_save);

  //Serial.println("sx12xx_receive");
  if (!sx12xx_receive_active) {
    if (settings->power_save & POWER_SAVE_NORECEIVE) {
      Serial.println("LMIC_shutdown");
  ...
  if (sx12xx_receive_complete == true) {
    ...
    if (memcmp(LMIC.frame+LMIC.protocol->payload_offset, TxBuffer, size) == 0)
    {
      //Serial.printf("RF  Rx <<- sz=%u t=%u ts=%u LOOP ===============================================\n",size,millis(),now());
      return false;
    }
#ifdef DUMPRFTRAFFIC
    Serial.printf("RF  Rx <<- sz=%u t=%u ts=%u ",size,millis(),now());
    for (u1_t i=0; i < size; i++)
      Serial.printf("%02x",LMIC.frame[i+LMIC.protocol->payload_offset]);
    Serial.println("");
#endif
...
static void sx12xx_rx_func (osjob_t* job) {
  ...
  if (LMIC.protocol->payload_offset!=0)
    LMIC.dataLen = RF_Payload_Size(settings->rf_protocol);
  else
    LMIC.dataLen=relay_framesize(LMIC.frame,LMIC.dataLen)+LMIC.protocol->crc_size;

#ifdef DUMPRFTRAFFIC
    Serial.printf("RF  RxDump <<- sz=%u t=%u ts=%u ",LMIC.dataLen,millis(),now());
    for (u1_t i=0; i < LMIC.dataLen; i++)
      Serial.printf("%02x",LMIC.frame[i]);
    Serial.println("");
#endif

  if (memcmp(LMIC.frame, TxBuffer, LMIC.dataLen) == 0)
  {
    Serial.printf("RF  Rx <<- sz=%u t=%u ts=%u LOOP ===============================================\n",LMIC.dataLen,millis(),now());
    return;
  }
...
//#if DEBUG
    if (crc16 == pkt_crc16 ) {
      //Serial.printf(" %04x is valid crc\n", pkt_crc16);
    } else {
      Serial.printf(" %04x is wrong crc\n", pkt_crc16);
    }
//#endif
    if (crc16 == pkt_crc16) {
      sx12xx_receive_complete = true;
...
static void sx12xx_tx(unsigned char *buf, size_t size, osjobcb_t func) {
  ...
  #if FALSE
  {
                ...
                //#if DEBUG
                    if (crc16 == pkt_crc16 ) {
                      Serial.printf(" TX %04x is valid crc\n", pkt_crc16);
                    } else {
                      Serial.printf(" TX %04x is wrong crc\n", pkt_crc16);
                    }
                //#endif
                    break;
                  }    
...
static void sx12xx_tx_func (osjob_t* job) {
  ...
  if (RF_tx_size > 0) {
#ifdef DUMPRFTRAFFIC
    Serial.printf("RF  Tx ->> sz=%u t=%u ts=%u ",RF_tx_size,millis(),now());
    for (u1_t i=0; i < RF_tx_size; i++)
