  ...
  void onWrite(BLECharacteristic *pUARTCharacteristic)
  {
    ...
    if (rxValue.length() > 0)
    {
      BLE_FIFO_RX->write(rxValue.c_str(),
                         (BLE_FIFO_RX->room() > rxValue.length() ? rxValue.length() : BLE_FIFO_RX->room()));
    }
  }
};
...
static void ESP32_Bluetooth_setup()
{
  ...
  case BLUETOOTH_LE_HM10_SERIAL:
  {
    ...
    const char *Model = hw_info.model == SOFTRF_MODEL_STANDALONE ? "Standalone Edition" : hw_info.model == SOFTRF_MODEL_PRIME_MK2 ? "Prime Mark II"
                                                                                                                                  : "Unknown";
    char SerialNum[9];
...
static void ESP32_Bluetooth_loop()
{
  ...
  case BLUETOOTH_LE_HM10_SERIAL:
  {
    ...
    if (deviceConnected && (millis() - BLE_Notify_TimeMarker > 10))
    { /* < 18000 baud */
...
BLESensBox::BLESensBox(void) : BLEService(SENSBOX_UUID_SERVICE),
...
void nRF52_Bluetooth_setup()
{
  ...
  bledis.setHardwareRev(hw_info.revision > 2 ? Hardware_Rev[3] : Hardware_Rev[hw_info.revision]);
