...
bool EPD_vmode_forcerefresh = true;
...
uint16_t EPD_pages_mask = (1 << VIEW_MODE_MAIN) | (1 << VIEW_MODE_STATUS) |
                          (1 << VIEW_MODE_RADAR) |
                          (1 << VIEW_MODE_TEXT) |
                          (1 << VIEW_MODE_TIME);
...
bool EPD_setup(bool splash_screen)
{
  ...
  if (hw_info.model == SOFTRF_MODEL_BADGE)
  {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "HW: %s SW: %s", hw_info.revision > 2 ? Hardware_Rev[3] : Hardware_Rev[hw_info.revision],
...
//      while (!SoC->Display_lock()) { delay(10); }
      ...
      {
#endif
        display->fillScreen(GxEPD_WHITE /* GxEPD_WHITE */);
...
//      SoC->Display_unlock();
    ...
    {
      if (EPD_vmode_forcerefresh)
        Serial.printf("EPD_vmode_forcerefresh\n");
      ...
      case VIEW_MODE_MAIN:
      default:
        EPD_main_loop(EPD_vmode_forcerefresh);
        break;
      }
      EPD_vmode_forcerefresh = false;
...
void EPD_fini(int reason, bool screen_saver)
{
  ...
  const char *msg = (reason == SOFTRF_SHUTDOWN_LOWBAT ? "LOW BATTERY" : "NORMAL OFF");
...
EPD_Task_t EPD_Task(void *pvParameters)
{
  ...
  for (;;)
  {
    if (EPD_update_in_progress != EPD_UPDATE_NONE)
    {
       ...
       */
      if (EPD_update_in_progress == EPD_UPDATE_FAST)
      { /* EPD_POWEROFF; */
      }
