...
//#define NRF52_JSON_BUFFER_SIZE  2048
...
static void nRF52_setup()
{
  ...
  if (nRF52_board == NRF52_LILYGO_TECHO_REV_2)
  {
    ...
    nRF52_board = Wire.endTransmission() == 0 ? nRF52_board : NRF52_LILYGO_TECHO_REV_0;
  }
  ...
  if (nRF52_has_spiflash)
  {
    ...
    Serial.printf("FATFS_is_mounted=%u\n", FATFS_is_mounted);
    /*if (!FATFS_is_mounted)
    {
      Serial.printf("FATFS formating\n");
      fatfs.format();
      Serial.printf("FATFS formated\n");
      FATFS_is_mounted = fatfs.begin(SPIFlash);
      Serial.printf("FATFS_is_mounted=%u\n",FATFS_is_mounted);
    }*/
  }
...
static void nRF52_post_init()
{
      ...
      nRF52_board == NRF52_LILYGO_TECHO_REV_2)
  {
    ...
    Serial.print(hw_info.revision > 2 ? Hardware_Rev[3] : Hardware_Rev[hw_info.revision]);
    ...
    Serial.flush();
    Serial.print(F("GNSS    : "));
    Serial.println(hw_info.gnss == ((hw_info.revision == 0 || hw_info.revision == 1) ? GNSS_MODULE_GOKE : GNSS_MODULE_AT65)
                       ? F("PASS")
                       : F("FAIL"));
    Serial.flush();
...
static void nRF52_loop()
{
  ...
  if (USBDevice.mounted() && usb_web.connected() && usb_web.available())
  {
    ...
    JsonObject &root = jsonBuffer.parseObject(usb_web);
...
static void nRF52_Sound_test(int var)
{
  ...
  if (SOC_GPIO_PIN_BUZZER != SOC_UNUSED_PIN && settings->volume != BUZZER_OFF)
  {
    ...
    delay(500);
    tone(SOC_GPIO_PIN_BUZZER, 640, 500);
    delay(500);
    tone(SOC_GPIO_PIN_BUZZER, 840, 500);
    delay(500);
...
static String nRF52_JSONSettings()
{
  String out;
  DynamicJsonBuffer jsonBuffer;
...
static void nRF52_EEPROM_extension(int cmd)
{
  ...
  case EEPROM_EXT_DEFAULTS:
    Serial.printf("EEPROM_EXT_DEFAULTS\n");
    ui->adapter = 0;
    ui->connection = 0;
      ...
      if (file)
      {
        ...
        if (root.success())
        {
          ...
          if (msg_class.success())
          {
            ...
            if (!strcmp(msg_class_s, "SOFTRF"))
            {
              Serial.printf("EEPROM_EXT_LOAD loaded\n");
              parseSettings(root);
              parseUISettings(root);
            }
          }
  ...
  case EEPROM_EXT_STORE:
    Serial.printf("EEPROM_EXT_STORE\n");
#if !defined(EXCLUDE_EEPROM)
    /*for (int i = 0; i < sizeof(eeprom_t); i++)
    ...
    if (nRF52_has_spiflash && FATFS_is_mounted)
    {
      ...
      if (file)
      {
        ...
        Serial.printf("EEPROM_EXT_STORE stored\n");
        file.close();
      }
      if (ADB_was_open)
        nRF52_ADB_setup();
...
static float nRF52_Battery_param(uint8_t param)
{
  ...
  case BATTERY_PARAM_THRESHOLD:
    rval = hw_info.model == SOFTRF_MODEL_BADGE ? BATTERY_THRESHOLD_LIPO : BATTERY_THRESHOLD_NIMHX2;
    break;

  case BATTERY_PARAM_CUTOFF:
    rval = hw_info.model == SOFTRF_MODEL_BADGE ? BATTERY_CUTOFF_LIPO : BATTERY_CUTOFF_NIMHX2;
    break;
...
{
  if (connected)
    usb_web.println("WebUSB Serial example");
}
#endif /* USE_WEBUSB_SERIAL */
