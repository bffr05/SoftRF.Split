...
bool Traffic_Parse(ufo_t &fo)
{
  ...
  for (i=0; i < MAX_TRACKING_OBJECTS; i++) {
    if (Container[i].addr == fo.addr) {
      ...
      float distance = gnss.distanceBetween( Container[i].latitude,
                                        Container[i].longitude,
                                        fo.latitude,
                                        fo.longitude);
      /*Serial.printf("distance %f ts=%u clat=%f clon=%f flat=%f flon=%f\n",distance,fo.timestamp-Container[i].timestamp,Container[i].latitude,
                                        Container[i].longitude,
                                        fo.latitude,
                                        fo.longitude);*/
      int diff = fo.timestamp-Container[i].timestamp;
      if (diff==0)
          diff=1;
      float kmh = (distance/1000*3600)/(diff);
      if (kmh> 400.0&&Container[i].refused_timestamp!=0)
      {
        distance = gnss.distanceBetween( Container[i].refused_latitude,
                                        Container[i].refused_longitude,
                                        fo.latitude,
                                        fo.longitude);
        diff = fo.timestamp-Container[i].refused_timestamp;
        if (diff==0)
          diff=1;
        kmh = (distance/1000*3600)/(diff);
      }
      if (kmh>400.0)
      {
